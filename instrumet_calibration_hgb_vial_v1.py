...
if __name__=='__main__':
    ...
    folderName = r"G:\Shared drives\Engineering\Optics_Sensing_Imaging\CC-Cyclops\Imrul_python_script_ISP\image_repo\betaFAT_v2\AS3\V2B_ronchi80A_run00"
    date = "20240710_3points"
    crop = True
    cropWindow = 1500
...
#%% quick calculation
    # X=np.array([500,750],dtype=float)
    # Y=np.array([799,1023],dtype=float) 
    # slope, intercept = np.polyfit(X, Y, deg=1)
    # slope = np.round(slope,3)
    # intercept = np.round(intercept,3)
