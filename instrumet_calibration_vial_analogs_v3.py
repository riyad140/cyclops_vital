...
def display_raw_histogram(imProbe,imSignal,showFigure=True):
    ...
    labels_g = detect_cells_cyclops5(im_g,501,-101,1200)    #(im_g,501,-5,1200)
...
if __name__=='__main__':
    ...
    folderName = r"W:\raspberrypi\photos\Alpha_sharp\CYC7_AS3\2024-12-06\analog_0"
    date = "20241209"
    crop = False
    cropWindow = 3000
    ...
    for fovCount in fovsToAnalyze:
        probeFilenames=[f"img_WBC_red_FLR_fov{fovCount}.tiff"]
        signalFilenames = [f"img_WBC_blue_DF_fov{fovCount}.tiff"] #[f"img_WBC_green_FLG_fov{fovCount}.tiff"]
        #probeFilenames
...
#         plt.savefig(os.path.join(resultPath,f"calibration_of_{fovCount}"+signalFilenames[-1][0:-10])+'.png')
...
#     df_final.to_csv(os.path.join(resultPath,date+'_all_fov_stat_of_'+signalFilenames[-1][0:-10])+'.csv')
...
#     df_final_summary.to_csv(os.path.join(resultPath,date+'SUMMARY_stat_of_'+signalFilenames[-1][0:-10])+'.csv')
#     print('SUMMARY')
#     print(dataSummary)
