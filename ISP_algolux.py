# -*- coding: utf-8 -*-
"""
Created on Wed Dec 15 05:13:23 2021
...
#%% fixing exposure
imGray=cv2.cvtColor(img_scaled, cv2.COLOR_BGR2GRAY)
imGray_8bit=(imGray//2**8).astype(np.uint8)
...
imBGR_exposed=(imBGR.astype(float)*digital_gain).clip(0,2**16-1).astype(np.uint16)
cv2.namedWindow('autoexposure', cv2.WINDOW_AUTOSIZE)
cv2.imshow('autoexposure',imBGR_exposed)


imBGR_exposed_8bit=(imBGR_exposed//2**8).astype(np.uint8)
plt.figure()
plt.imshow(imBGR_exposed_8bit)

cv2.namedWindow('autoexposure8', cv2.WINDOW_AUTOSIZE)
cv2.imshow('autoexposure8',imBGR_exposed_8bit)
...
#%% denoising
bilateral = cv2.bilateralFilter(imBGR_exposed.astype(np.float32), 10, 10*256, 10).astype(np.uint16).astype(np.float32)


bilateral_8bit = cv2.bilateralFilter(imBGR_exposed_8bit, 10, 10, 10)
plt.figure()
plt.imshow(bilateral_8bit[:,:,2])
...
def percentile_whitebalance(image, percentile_value,bit_depth=8):
    ...
    # whitebalanced =( ((image*1.0 / np.percentile(image,percentile_value, axis=(0, 1))).clip(0, 1))*(2**8-1)).astype('uint8')
...
bilateral_8bit_wb=percentile_whitebalance(bilateral_8bit, percentile_value=99.9)
...
imGamma_8bit=adjust_gamma(bilateral_8bit_wb,gamma=1.2)
plt.figure()
