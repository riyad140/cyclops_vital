...
if __name__ == "__main__":
    file=r"G:\Shared drives\Engineering\Optics_Sensing_Imaging\CC-Cyclops\Imrul_python_script_ISP\image_repo\2021-01-21_SensorCharacterization\usaf target\IMX477\cyclops-G_BF-FOV_0.png"
    im=cv2.imread(file,0)
    # a,b,c=Compute(im, edge_start=np.array((2533,1680)), edge_end=np.array((2279,1215)), desired_width=200, crop_ratio=0., use_50p=True)
    patch = _ExtractPatch(im, edge_start=np.array((1542,901)), edge_end=np.array((1540,1020)), desired_width=100, crop_ratio=0)
    angle, centers = _FindEdgeSubPix(patch, desired_width=100)
    plt.figure()
    plt.imshow(patch)
    plt.figure()
    plt.plot(centers)
