...
def get_peaks_troughs(crossSection,trimSize=4,prominence=200,distance=10):  # finds the peaks and troughs 
...
def get_peaks_analysis(crossSections,keys=['ctr_ctr'],key_angle='000'):
    ...
    for count,key in enumerate(keys):
        ...
        figName=f'cross_section_angle_{key_angle}.png'
...
df_table.append(df_table_135)
#%% QC check
dataList=[]
for count,df in enumerate(df_table):
    sliceAngle=list(df['Slice_angle'])[0]
    meanContrast= np.nanmean(list(df['Central_Contrast']))
    stdContrast= np.nanstd(list(df['Central_Contrast']))
    meanDifference=np.sum(list(df['Trough_Contrast']))
    
    data={'Slice_Angle': sliceAngle,
          'Mean_Contrast': np.round(meanContrast,1),
          'Std_Contrast': np.round(stdContrast,1),
          'Mean_Difference': np.round(meanDifference,1)  
    ...
    # else:
    #     df_qc.append(pd.DataFrame(data))
df_qc=pd.DataFrame(dataList)    




#%% Plotting the Cropped Beads
n=len(keys)
nr,nc=n//3,n//3
...
plt.gcf()
figName='cropped_images_across_fovs.png'
plt.savefig(os.path.join(resultPath,figName))
#%% saving to excel



filename='contrast_analysis.xlsx'
excelFileName=os.path.join(resultPath,filename)

with pd.ExcelWriter(excelFileName) as writer:  
    for df in df_table:
    
        df.to_excel(writer, sheet_name=list(df['Slice_angle'])[0])
    df_qc.to_excel(writer, sheet_name='QC')
