...
def smartAFRoutine(fov,centerZ=500,resolutionZ=5,maxIter=3,channel='R'):
    ...
    if AFpin=='DF':
        ...
        gpio.output(BFLEDPin, 1) # BF light
...
def run_oneFOV(fov): # capturing images for one fov
    ...
    if disable_AF is False:
        dt_AF,sharpest_VCM= dt_AF1,sharpest_VCM=smartAFRoutine(fov=0,centerZ=500,resolutionZ=5,maxIter=3,channel='R')#automateAFRoutine(fov, defaultZstep, defaultZrange, defaultZresolution,channel=AF_camera)
    else:
        logging.info('AF disabled')
