...
def normalize_data(df,quantiles=[0.7,0.1,0.4]):
    ...
    return df
#%%
FL = sorted(glob.glob(os.path.join(path,'*G_FL_RT-FLGRN_550-60_FOV_1*')))  # #-G_FL-
BGDF = sorted(glob.glob(os.path.join(path,'*-G_bg_DF-*'))) #'*-G_bg_DF-*'
DF = sorted(glob.glob(os.path.join(path,'*G_FL_T-DF_480-40_FOV_1*')))  #*-G_DF-*
extension='bmp'
disable_BGDF=True
key0='FOV_'
key1='.'+extension
...
for i in range(len(FL)) :
    ...
    if i not in fovs_to_ignore:
        ...
        label_erosion = 0
        expand = 0
        labels_green = detect_cells(img_FL,th_block=81,th_offset=0,th_size=50,sigma=3,erosion=label_erosion) #default th_block 81
        ...
        regprops_ssbg_df['radius_filter']=regprops_ssbg_df['radius']<1300
...
for index,row in df_new.iterrows():
    ...
    row['fix_area_mean']=np.nanmedian(imCrop0) # originally np.nanmean was used
...
data.append(ax[0].hist(df_include['image_median_ss-bg'],bins=100, range=(hist_range[0],hist_range[1])))
ax[0].set_title('Contour_Mean')
ax[0].set_xlabel('SS')
ax[0].set_ylabel('Frequency')
#Plot histogram 
# plt.figure()
data.append(ax[1].hist(df_include['image_median_ss-bg_corr'],bins=100,range=(hist_range[0],hist_range[1])))
...
fig,ax=plt.subplots(1,2)
data.append(ax[0].hist(df_new['fix_area_mean'],bins=100,range=(hist_range[0],hist_range[1])))
ax[0].set_title('Fixed_area_Mean')
ax[0].set_xlabel('SS')
ax[0].set_ylabel('Frequency')
#Plot histogram 
# plt.figure()
data.append(ax[1].hist(df_new['fix_area_std'],bins=100,range=(hist_range[0],hist_range[1])))
