...
WARNING: GLASS FL BASED CALIBRATION DO NOT SEEM TO WORK. PS BEADS remains the best way
...
def histogram_analysis(xHist,yHist,figTitle='gg'):
    ...
    peaks, _ = signal.find_peaks(yHist1,prominence=8,distance=100)
...
# print(targetX)
def get_robust_median(im_g, backgroundPercentile = 25):
...
if __name__=='__main__':
    ...
    folderName = r"Y:\Cyclops Backups\FAT_Captures\BetaFATv2\AS01\2024-06-10\glassFL"
    date = "20240604"
    crop = True
    cropWindow = 1500
    ...
    for fovCount in fovsToAnalyze:
        probeFilenames=[ f"img_FAT_glassFL_green_FLG_ledPWR_500.tiff",f"img_FAT_glassFL_green_FLG_ledPWR_750.tiff",f"img_FAT_glassFL_green_FLG_ledPWR_1000.tiff"]
        signalFilenames =probeFilenames
        # [ f"img_blue_DF_fov{fovCount}_intensityB_500.tiff",f"img_blue_DF_fov{fovCount}_intensityB_750.tiff",f"img_blue_DF_fov{fovCount}_intensityB_1000.tiff"]
        #probeFilenames
        ...
        for i in range(len(signalFilenames)):
            ...
            meanInt,_ = get_robust_median(imSignal,25)#np.median(imSignal)
