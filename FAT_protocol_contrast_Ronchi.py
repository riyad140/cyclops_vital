...
#%%
def get_contrast(im_main,initial_coords,binPath,resultPath,rot_angle=60,name='gg',closeFigure=False):
    # im_main=ims[0]
    ...
    im_rotated = ndimage.rotate(im_snip, rot_angle,reshape=False,order=1)
    ...
    # plt.plot(xSection)
    #%
    peaks, _ = signal.find_peaks(xSection,prominence=50,distance=25)
    troughs,_=signal.find_peaks(-xSection,prominence=50,distance=25)
    plt.figure()
    plt.plot(xSection)
...
#%%
binPath=r'G:\Shared drives\Engineering\Optics_Sensing_Imaging\CC-Cyclops\Imrul_python_script_ISP\image_repo\Baseline_capture_for_Volpi_Visit\Captures_at_Volpi\Capture_by_Mikolaj'
...
for i,coords in enumerate(coords_list):
    con,pdr,mag=get_contrast(im_main,coords,binPath,resultPath,rot_angle=rotation_angle,name='sector_'+str(i),closeFigure=True)
...
plt.figure()
plt.imshow(con_map,cmap='jet_r',vmin=55,vmax=80)
plt.colorbar()
...
plt.figure()
plt.imshow(pdr_map,cmap='jet_r',vmin=120,vmax=150)
plt.colorbar()
