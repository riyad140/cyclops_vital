...
disable_motor= True
defaultFOVrange = 10 #number of FOVs to examine for the test, minimum value 1
...
imWidth,imHeight=4056,3040
defaultShutterSpeed=50e3 #us 
raw_capture=False # to enable raw capture
HDR=False  # to enable 3 frames at different exposure instead of 1 frame
########################### BF DF SS FL FL_G#########################
exif_data={'shutter_speed':[0.5,1400,2000,600, 150], 
...
params={'width':' -w '+str(imWidth),
        'height':' -h '+str(imHeight),
        'white_balance':' -awb off -awbg 1.0,1.0',
        'exposure':' -ex off',
        'shutter_speed':' -ss 500000',
        'gain':' -ag 1.0 -dg 1.0',
        'raw_capture':defaultRaw,
        'hdr': False,
        'hdr_factor': 2,
        'delay':' -t 1',
        'preview': '',
...
params_BF={
    'shutter_speed':' -ss '+str(exif_df['BF']['shutter_speed']*1e3),
    'gain':' -ag '+str(exif_df['BF']['analog_gain'])+' -dg 1.0' , 
    }

params_DF={
    'shutter_speed':' -ss '+str(exif_df['DF']['shutter_speed']*1e3),
    'gain':' -ag '+str(exif_df['DF']['analog_gain'])+' -dg 1.0' , 
    }
params_FL={
    'shutter_speed':' -ss '+str(exif_df['FL']['shutter_speed']*1e3),
    'gain':' -ag '+str(exif_df['FL']['analog_gain'])+' -dg 1.0' , 
    }

params_AF={
    'width':' -w '+str(imWidth//4),
    'height':' -h '+str(imHeight//4),
    'shutter_speed':' -ss '+str(exif_df['BF']['shutter_speed']*1e3),
    'gain':' -ag '+str(exif_df['BF']['analog_gain'])+' -dg 1.0' , 
    'raw_capture':'',
    'hdr':False,
...
#%% Functions to drive the workflow
...
def fm_lape(im):
    ...
    fm=np.mean(abs(fm))
    return(fm)
#Function to calculate sharpness based on non zero gaussian
...
def runVCM(final_vcm,init_vcm=0): #slowly drive the vcm
    ...
    for i in np.linspace(init_vcm,final_vcm,abs(final_vcm-init_vcm)+1):
        driveVCM(int(i))
#     sleep(0.25)
        

    
def capturePreview(Channel='R'):
    light_ON()
    selectCamera(Channel)
    cmd = "raspistill -t 0 -awb greyworld -o capture_RED" + ".bmp"
    os.system(cmd)
    filename = defaultFname + "Alignment.bmp"     
    #after alignment, capture an image to compare with the AF and BF images    
    cmd= "raspistill -t 1 -awb greyworld -o " + filename
    os.system(cmd)
    light_OFF()

def captureAFImage(stepcount, zindex,params={}):
    ...
    Gain=params['gain']
    Raw_Capture=params['raw_capture']
    Preview=params['preview']
    Exif=params['exif']
    Delay=params['delay']
    
    cmd = "raspistill" + Width + Height + Shutter_Speed + Gain + Exposure + Delay+ White_Balance+ Raw_Capture + " -o " + filename
...
def smartAFRoutine(fov,centerZ=defaultZstep,resolutionZ=5,maxIter=3,channel='R'):
...
#     runVCM(vcm0,defaultZstep)
    driveVCM(vcm0)
    selectCamera(channel)
    filename=captureAFImage(fov,vcm0,{**params,**params_AF})    
...
#             runVCM(v,last_vcm)
            driveVCM(v)
            last_vcm=v
            filename=captureAFImage(fov,v,{**params,**params_AF})    #captureImage({**params,**params_FL},channel='R',ledType='FL',fov=step)
            s=getSharpness(filename)
            v_s.append([v,s])         
    ...
    logging.info('Driving VCM to ultimate sharpest plane')
#     runVCM(vcm0,last_vcm)
    driveVCM(vcm0)
    t1=time.time()
    dt=t1-t0
    light_OFF()
...
def light_ON(ledType='BF'):
    if ledType=='BF':
        ...
        logging.info(f'LED of {ledType} not recognized')
...
#         print("Calling AF image Capture")
        captureAFImage(step, zindex,{**params,**params_AF}) #capture the BF Images for coarse AF investigation
...
def captureImage(params,channel='R',ledType='FL', fov=0,  filename='capture.png',vcm_shift=0,flip=False):
    t0=time.time()
    logging.info(f'Capturing {ledType} image with vcm offset of {vcm_shift} unit')
    if abs(vcm_shift)>0:
    ...
    filename = defaultFname + channel + "_"+ledType+"-"  + 'FOV_'+ str(fov) + ".png"
    logging.info(f'capturing {ledType} image')
    ...
    Delay=params['delay']
    HDR=params['hdr']
    HDR_Factor=params['hdr_factor']
    
    cmd = "raspistill" + Width + Height + Shutter_Speed + Gain + Exposure + Delay+ White_Balance+ Raw_Capture + suffix + " -o " + filename
    os.system(cmd)
    
    if HDR is True:
        logging.info('Taking HDR images')
        filename_lo = defaultFname + channel + "_"+ledType+"-"  + 'FOV_'+ str(fov) + "_lo.png"
        cmd = "raspistill" + Width + Height + Shutter_Speed//HDR_Factor + Gain + Exposure + Delay+ White_Balance+ Raw_Capture + suffix + " -o " + filename_lo
        os.system(cmd)
        filename = defaultFname + channel + "_"+ledType+"-"  + 'FOV_'+ str(fov) + "_hi.png"
        cmd = "raspistill" + Width + Height + Shutter_Speed*HDR_Factor + Gain + Exposure + Delay+ White_Balance+ Raw_Capture + suffix + " -o " + filename_hi
        os.system(cmd)            
...
def automateAnalysisRoutine(step):
    ...
    dt_BF,filename_BF=captureImage({**params,**params_BF},channel='R',ledType='BF',fov=step)
    dt_DF,filename_DF=captureImage({**params,**params_DF},channel='R',ledType='DF',fov=step)
    dt_FL,filename_FL=captureImage({**params,**params_FL},channel='R',ledType='FL',fov=step)
    
    logging.info("Selecting camera B")
    logging.info("Capturing Green Channel images")
    selectCamera('G')
    dt,filename_FL_G=captureImage({**params,**params_FL},channel='G',ledType='FL',fov=step,flip=True)
...
async def setHome():
    logging.info('setting home to here')
    on_alert = lambda c: ""
...
#         for i in range (0,(defaultZstep + 1)):
    runVCM(defaultZstep) #begin by driving the VCM to the default position defined above
    sleep(1)
...
#     flatfieldCaptures() #call an alignment function to select a region for FF Image capture
    if disable_AF is False:
        ...
        dt_AF1,sharpest_VCM=smartAFRoutine(fov=0,centerZ=500,resolutionZ=50,maxIter=3,channel='R')
    else:
...
def run_oneFOV(fov): # capturing images for one fov
    ...
    if disable_AF is False:
        dt_AF,sharpest_VCM= smartAFRoutine(fov=0,centerZ=defaultZstep,resolutionZ=5,maxIter=3,channel='R')#automateAFRoutine(fov, defaultZstep, defaultZrange, defaultZresolution,channel=AF_camera)
    else:
        logging.info('AF disabled')
