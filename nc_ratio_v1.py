...
def calculate_cn_ratios_and_areas(green_channel_path, red_channel_path):
    ...
    for region in regions:
        ...
        cytoplasm_region = red_mask[min_row:max_row, min_col:max_col]
        ...
        cell_nucleus_area = cell_combined_area - cell_cytoplasm_area

        # Skip if the cytoplasm area is zero or negative
        if cell_nucleus_area <= 10 or cell_cytoplasm_area <=10:
            continue
...
try:
    ...
    plt.hist(np.array(cn_ratios), bins=400, color='blue', edgecolor='black')
