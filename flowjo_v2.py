...
if __name__ == "__main__":
    ...
    file_extension = ".fcs"  # Change this to the desired file extension

    file_paths = find_subfolders_with_extension(root_directory, search_string, file_extension)
    ...
    for fcs_file_path in file_paths:
        ...
        df_temp  = manual_gating(fcs_file_path)
        df_temp['path'] = fcs_file_path
        df_master_record.append(df_temp)
        
        
    df_master_record = pd.concat(df_master_record)
    df_master_record.to_csv(os.path.join(root_directory,'Manual Gating Stat Summary.csv'))    
